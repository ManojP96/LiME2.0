...
if auth_status:
    ...
    llm = HuggingFaceHub(
        ...
        huggingfacehub_api_token="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"
    )
    ...
    st.markdown(
        ...
        **Welcome to LiME**
        ...
        LiME is your all-in-one solution for media mix modeling, boasting nine intuitive tabs that cater to various functionalities. Dive into constructing MMM models, analyzing, optimizing, and visualizing spends, contributions, and scenarios across multiple media channels effortlessly.

        Whether you're seeking to fine-tune your media strategies or maximize your marketing efforts,  LiME empowers you to make informed decisions backed by robust data analysis and visualization.
        """
    )
    ...
    # question=st.selectbox('',questions['Question'])


    user_text = st.text_input(label='',placeholder="Query: Type your query or select any option from the dropdown below", key='input')
    question = st.selectbox('Select a Query', questions['Question'],index=None,)

    def reset():
        st.session_state.question='Select a Query'

    columns=st.columns(5)
    with columns[0]:
        submit_button = st.button(label='Send',on_click=reset)
    ...
    if submit_button and (user_text or question != 'Select a Query'):
        if user_text:
